...
Note that I have not figured out how RSparsePro can be integrated with a 
cTWAS-style framework (or if that is feasible with minimal modification),
so right now RSparsePro is run just once, with no class priors.

This obtained solid results in simulations with no need for strong priors on 
factors or rescaling Z-scores, as well as in real data (see ``Metabolite MR'').
...
Also, the PIPs tend to be very dichotomous, i.e. almost always very close to 0 or 1.
