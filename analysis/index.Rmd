...
group_barchart = function(dirname, prefix='all_res_', n_x_show = 8, thresh = 0.05,
  ...
  for(m in methods) {
    ...
    for (i in 1:n_x_show) {
      if(m == 'susie' || m == 'zuber') {
        posrate = sum(res[,i] <= thresh) / length(res[,i])
      } else {
  ...
  ggplot(data, aes(fill=method_labels, y=all_pos_rates, x=beta_labels)) +
    ...
    xlab('Exposures') + ylab('Positive Rate') + labs(fill = "Method")
}


param_lineplot = function(dirvec, param_name, param_vals, truepos, prefix='all_res_', thresh = 0.05,
                            methods=c('2sls', '2sls_oracle', 'ivw', 'egger', 'median', 'lasso')) {
  all_method_labels = c()
  all_param_vals = c()
  all_fpr = c()
