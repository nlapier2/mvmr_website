...
group_barchart = function(dirname, prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.1,
                          methods=c('2sls', '2sls_oracle', '2sls_enahnced', '2sls_cate', 'susie_svd', 'ivw', 'egger', 'median', 'lasso')) {
  method_labels = c()
  beta_labels = c()
  all_pos_rates = c()
...
power_fpr_thresh_plot = function(dirname, truevars, prefix='all_res_', n_x_show = 4, thresh = 0.05,
                          methods=c('2sls', '2sls_oracle', '2sls_enahnced', 'ivw', 'egger', 'median', 'lasso')) {
  method_labels = c()
  beta_labels = c()
  all_pos_rates = c()
...
(UPDATE) Here are some results including the "2SLS Enhanced" and "SuSiE SVD" methods.

Here are the results with the default settings.
2SLS Enhanced seems to achieve more separation between power and FPR than other methods (except 2SLS Oracle),
although it is still inflated.

```{r both_plei_full, echo=FALSE, fig.width=12}
group_barchart('data/results_2sls_enhanced/both_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05,
               methods=c('2sls', '2sls_oracle', '2sls_enhanced', 'susie_svd', 'ivw', 'egger', 'median', 'lasso'))
```

Here are the powers to detect each true variable at an FPR threshold of 0.05:

```{r both_plei_powthresh, echo=FALSE, fig.width=12}
power_fpr_thresh_plot ('data/results_2sls_enhanced/both_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               '1,2,3,4', prefix='all_res_', n_x_show = 4, thresh = 0.05,
               methods=c('2sls', '2sls_oracle', '2sls_enhanced', 'ivw', 'egger', 'median', 'lasso'))
```


As a sanity check, the plot below shows results in simulations with no confounding. 
As expected, no methods are inflated.
However, 2SLS Enhanced has much lower power than other methods for weak true effects.

```{r no_plei_full, echo=FALSE, fig.width=12}
group_barchart('data/results_2sls_enhanced/no_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05,
               methods=c('2sls', '2sls_oracle', '2sls_enhanced', 'susie_svd', 'ivw', 'egger', 'median', 'lasso'))
```

And again, here are the powers to detect each true variable at an FPR threshold of 0.05, showing similar trends
to the barchart above:

Here are the powers to detect each true variable at an FPR threshold of 0.05:

```{r no_plei_powthresh, echo=FALSE, fig.width=12}
power_fpr_thresh_plot ('data/results_2sls_enhanced/no_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               '1,2,3,4', prefix='all_res_', n_x_show = 4, thresh = 0.05,
               methods=c('2sls', '2sls_oracle', '2sls_enhanced', 'ivw', 'egger', 'median', 'lasso'))
```

(UPDATE 2) Here are some results with 2SLS-Cate in the only-correlated-pleiotropy case:

```{r cor_plei_cate, echo=FALSE, fig.width=12}
group_barchart('data/results_2sls_cate/cor_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05,
               methods=c('2sls', '2sls_oracle', '2sls_cate'))
```
...
```{r cor_plei_cate_powthresh, echo=FALSE, fig.width=12}
power_fpr_thresh_plot ('data/results_2sls_cate/cor_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               '1,2,3,4', prefix='all_res_', n_x_show = 4, thresh = 0.05,
               methods=c('2sls', '2sls_oracle', '2sls_cate'))
```


(UPDATE 3) Here are some results with "susiecate" -- Cate results included in Susie -- with L=60:

No pleiotropy:

```{r no_plei_susiecate, echo=FALSE, fig.width=12}
group_barchart('data/results_susiecate_L60/no_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.5,
               methods=c('2sls', '2sls_oracle', '2sls_cate', 'susie_svd'))
```

Correlated pleiotropy only:

```{r cor_plei_susiecate, echo=FALSE, fig.width=12}
group_barchart('data/results_susiecate_L60/cor_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.5,
               methods=c('2sls', '2sls_oracle', '2sls_cate', 'susie_svd'))
```

Uncorrelated pleiotropy only:

```{r uncor_plei_susiecate, echo=FALSE, fig.width=12}
group_barchart('data/results_susiecate_L60/uncor_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.5,
               methods=c('2sls', '2sls_oracle', '2sls_cate', 'susie_svd'))
```

Both correlated and uncorrelated pleiotropy:

```{r both_plei_susiecate, echo=FALSE, fig.width=12}
group_barchart('data/results_susiecate_L60/both_plei_gx_0.2_gy_0.2_gz_0.4_psix_0.2_psiy_0.15_mu_0/', 
               prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.5,
               methods=c('2sls', '2sls_oracle', '2sls_cate', 'susie_svd'))
```
