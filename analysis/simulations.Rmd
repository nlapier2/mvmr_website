...
group_barchart = function(dirname, prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.5, methods=c()) {
  ...
  for(m in methods) {
    ...
    if(grepl('susie', ss) || grepl('mrash', ss) || grepl('zuber', ss)) {
      num_betas = num_cols 
    } else {
...
So there's two interesting things going on here:

* Robust seems to cause false positives when the number of exposures 
approaches the number of SNPs
* Methods seem to not be inflated under pleiotropy, even without any first stage FA.
I am less sure why this is. Maybe the sparsity of the true effects and the low
number of confounders relative to the number of exposures?

It could be worth investigating stronger confounding effects and denser settings
to see if that can cause inflation in the methods.
