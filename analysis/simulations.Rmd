...
group_barchart = function(dirname, prefix='all_res_', n_x_show = 8, thresh = 0.05, pip_thresh = 0.1,
  ...
  for(m in methods) {
    ...
    num_cols = dim(res)[2]
    if(m == 'susie_svd' || m == 'susiecate' || m == 'zuber') {
      num_betas = num_cols 
    } else {
...
<!-- Here's the performance when we vary $\theta_{GX}$, the part of $X$'s heritability NOT mediated through $Z$. As expected, power increases as the heritability of $X$ increases. Interestingly, so does the FPR (except for 2SLS-Oracle). -->
